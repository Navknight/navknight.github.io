...
import { Briefcase, Calendar, ShieldCheck, Cpu, Code2, Database, Layout } from 'lucide-react'
...
const projects = [
  {
    ...
    tech: ['C++', 'Objective-C', 'Chromium Source', 'Android', 'Shell Scripting']
  },
  {
  ...
  },
  {
    ...
    tech: ['Python', 'Flask', 'Google API', 'OneDrive API', 'Box SDK', 'Docker']
  },
  {
...
export default function Experience() {
  ...
  return (
    <section id="experience" ref={sectionRef} className="section-container relative">
      {/* Background glow */}
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <div className={`lg:col-span-4 glass rounded-2xl p-6 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-[11px] text-text-muted">via SquareX Acquisition</p>
            </div>
          </div>
          ...
          <p className="text-xs text-text-secondary leading-relaxed mb-6">
            Driving browser security and low-level posture capabilities inside enterprise cloud and extension products. Select modules on the right to examine core contributions.
          </p>
          ...
          <div className="flex flex-col gap-2">
            {projects.map((proj, idx) => {
              ...
              return (
                ...
                >
                  <Icon size={14} className={activeIdx === idx ? 'text-accent-indigo' : 'text-text-muted'} />
        ...
        {/* Detailed Work Panel */}
        <div className={`lg:col-span-8 glass rounded-2xl p-8 min-h-[380px] flex flex-col justify-between transition-all duration-700 delay-100 ${
          ...
          <div>
            ...
            <ul className="space-y-4">
              {projects[activeIdx].details.map((bullet, bIdx) => (
                <li key={bIdx} className="flex gap-3 text-sm text-text-secondary leading-relaxed animate-fade-up" style={{ animationDelay: `${bIdx * 50}ms` }}>
                  <span className="text-accent-indigo mt-1 shrink-0 font-mono text-[10px]">▸</span>
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
